...
class TestLoginFlow:
    ...
    async def test_successful_login_redirects_to_dashboard(
    ...
    async def test_sql_injection_safely_rejected(
        self, login_page: LoginPage
    ) -> None:
        await login_page.login("' OR '1'='1", "' OR '1'='1")
        await login_page.expect_error_visible()

    async def test_xss_in_username_safe(
        self, login_page: LoginPage
    ) -> None:
...
class TestDashboard:
    ...
    async def test_dashboard_shows_welcome_after_login(
    ...
    async def test_logout_returns_to_login(
        self, page: Page, base_url: str
    ) -> None:
        lp = LoginPage(page)
        await page.goto(f"{base_url}/login")
    ...
    async def test_direct_dashboard_access_without_auth(
        self, page: Page, base_url: str
    ) -> None:
        """
...
class TestLoginPageUI:
    ...
    async def test_login_form_elements_present(
        self, login_page: LoginPage
    ) -> None:
