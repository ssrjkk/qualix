...
class TestCreateUserRouter:
    ...
    async def test_success_returns_user_response(self) -> None:
        ...
        with patch("app.api.users.UserRepository") as R:
            R.return_value.create = AsyncMock(return_value=mock_user)
    ...
    async def test_integrity_error_raises_409(self) -> None:
        mock_db = AsyncMock()
        with patch("app.api.users.UserRepository") as R:
            R.return_value.create = AsyncMock(side_effect=IntegrityError("dup", {}, None))
    ...
    async def test_response_has_no_password_field(self) -> None:
        mock_db = AsyncMock()
        with patch("app.api.users.UserRepository") as R:
            R.return_value.create = AsyncMock(return_value=_make_user())
            result = await users_module.create_user(
...
class TestListUsersRouter:
    ...
    async def test_returns_list_response(self) -> None:
        ...
        with patch("app.api.users.UserRepository") as R:
            R.return_value.list = AsyncMock(return_value=users)
            R.return_value.count = AsyncMock(return_value=5)
    ...
    async def test_pagination_params_passed_correctly(self) -> None:
        ...
        with patch("app.api.users.UserRepository") as R:
            ...
            R.return_value.count = AsyncMock(return_value=100)
            result = await users_module.list_users(
                limit=5, offset=10, db=mock_db, _={"username": "admin"}
            )
        assert result.limit == 5
        assert result.offset == 10
        R.return_value.list.assert_awaited_once_with(limit=5, offset=10)

    async def test_empty_list_returns_valid_response(self) -> None:
        mock_db = AsyncMock()
        with patch("app.api.users.UserRepository") as R:
            R.return_value.list = AsyncMock(return_value=[])
            R.return_value.count = AsyncMock(return_value=0)
            result = await users_module.list_users(
                limit=20, offset=0, db=mock_db, _={"username": "admin"}
            )
        assert result.items == []
...
class TestGetUserRouter:
    ...
    async def test_found_returns_user_response(self) -> None:
        mock_db = AsyncMock()
        with patch("app.api.users.UserRepository") as R:
            R.return_value.get_by_id = AsyncMock(return_value=_make_user(42))
            result = await users_module.get_user(
                user_id=42, db=mock_db, _={"username": "admin"}
            )
        assert isinstance(result, UserResponse)
        assert result.id == 42
    ...
    async def test_not_found_raises_404(self) -> None:
        mock_db = AsyncMock()
        with patch("app.api.users.UserRepository") as R:
            R.return_value.get_by_id = AsyncMock(return_value=None)
            with pytest.raises(HTTPException) as exc:
                await users_module.get_user(
                    user_id=999, db=mock_db, _={"username": "admin"}
                )
...
class TestDeleteUserRouter:
    ...
    async def test_success_returns_none(self) -> None:
        mock_db = AsyncMock()
        with patch("app.api.users.UserRepository") as R:
            R.return_value.delete = AsyncMock(return_value=True)
            result = await users_module.delete_user(
                user_id=1, db=mock_db, _={"username": "admin"}
            )
    ...
    async def test_not_found_raises_404(self) -> None:
        mock_db = AsyncMock()
        with patch("app.api.users.UserRepository") as R:
            R.return_value.delete = AsyncMock(return_value=False)
            with pytest.raises(HTTPException) as exc:
                await users_module.delete_user(
                    user_id=999, db=mock_db, _={"username": "admin"}
                )
        assert exc.value.status_code == 404
